...
const StockEntryForm = ({ activeStock, onAdd }) => {
  ...
  return (
    <form onSubmit={handle} className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm grid grid-cols-1 md:grid-cols-3 gap-8">
      <div className="flex flex-col gap-3">
        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Nome do Corte</label>
        <input type="text" className="bg-slate-50 p-4 rounded-2xl text-xs outline-none focus:ring-2 focus:ring-blue-500 transition-all border-none" value={produto} onChange={e => setProduto(e.target.value)} required />
      </div>
      <div className="flex flex-col gap-3">
        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Peso Entrada (kg)</label>
        <input type="number" step="0.01" className="bg-slate-50 p-4 rounded-2xl text-xs outline-none focus:ring-2 focus:ring-blue-500 transition-all border-none" value={qtd} onChange={e => setQtd(e.target.value)} required />
      </div>
