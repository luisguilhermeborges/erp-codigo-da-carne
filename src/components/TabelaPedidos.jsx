    ...
    <div className="p-8 border-b border-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-6">
      <div className="flex items-center gap-3">
        ...
        <h3 className="font-bold text-slate-700 uppercase text-xs tracking-widest">
          Inventário: <span className="text-blue-600">{nomeEstoque}</span>
        </h3>
      </div>
          ...
          {itens.map(i => (
            <tr key={i.id} className="text-xs hover:bg-blue-50/30 transition-colors">
              ...
              <td className="px-10 py-5 text-center">
                <button onClick={() => aoExcluir(i.id)} className="p-2 text-slate-300 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all">
                  <Trash2 size={18} />
                </button>
              </td>
            </tr>
