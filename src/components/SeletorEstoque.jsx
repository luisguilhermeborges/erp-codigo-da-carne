import React from 'react';
import { MapPin } from 'lucide-react';

const SeletorEstoque = ({ estoqueAtivo, setEstoqueAtivo }) => {
  // UNIDADES: Lista das lojas. Caso abra uma nova unidade, basta adicionar nesta lista.
  const unidades = ['matriz', 'alphaville', 'gleba'];

  return (
    <div className="flex gap-3 mb-8 bg-white p-2 rounded-2xl border border-slate-100 w-fit shadow-sm">
      {unidades.map(local => (
        <button 
          key={local} 
          onClick={() => setEstoqueAtivo(local)} 
          className={`px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${
            // DESTAQUE: Aplica o estilo de botão ativo.
            estoqueAtivo === local 
              ? 'bg-blue-600 text-white shadow-lg shadow-blue-100' 
              : 'text-slate-400 hover:bg-slate-50'
          }`}
        >
          <MapPin size={14} className={estoqueAtivo === local ? "text-blue-200" : "text-slate-300"} />
          {local}
        </button>
      ))}
    </div>
  );
};

export default SeletorEstoque;