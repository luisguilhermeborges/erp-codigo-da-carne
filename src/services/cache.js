...
export const getEstoque = async ({ apenasComPreco = false, apenasLoja = false } = {}) => {
  const precos = await getPrecos();
  const bancoUsado = apenasLoja ? BANCO_PADRAO : BANCO_COMPLETO;
  const lista = Object.entries(bancoUsado).map(([nome, v]) => {
